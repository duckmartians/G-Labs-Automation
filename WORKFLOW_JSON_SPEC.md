...
Data types of the single output socket are shown as **→ type**.

#### `prompt` — a text prompt  → `string`
- **inputs:** `[]` (none). **outputs:** `["Prompt"]`.
- **values:** `{ "prompt": "<your text>" }`

#### `batch_prompt` — many prompts, loops the downstream chain once per line  → `string`
- **inputs:** `[]`. **outputs:** `["Prompt"]`.
- **values:**
  - `prompt_list` (string, multi-line — one prompt per line)
  - `mode` (index-based combo: `0`=Sequential, `1`=Random — stored as display text; omit to keep default Sequential)
  - `limit_mode`, `limit_val` (optional cap; omit to use all lines)

#### `reference` — an image file from disk  → `image`
- **inputs:** `[]`. **outputs:** `["Image"]`. Add `"ref_mode": "pro"`.
- **values:** `{ "file_path": "C:/absolute/path/to/image.png" }` (the file must exist on the machine running the app).

#### `batch_loader` — a folder of images, loops once per image  → `image`
- **inputs:** `[]`. **outputs:** `["Image"]`.
- **values:** `file_path` (folder path); `sort_mode`, `limit_mode`, `limit_val` (optional; index-based combos, omit to keep defaults).
...
- **values:**
  - `model` (data value — a server-provided Flow model key; **see note below**)
  - `ratio` (model-dependent, e.g. `IMAGE_ASPECT_RATIO_LANDSCAPE`, `..._PORTRAIT`, `..._SQUARE`)
  - `res_group` (button text: `"1K"`, `"2K"`, or `"4K"` — 2K/4K need ULTRA + upscale-capable model)
  - `seed_mode` (index: 0=random, 1=fixed), `seed` (text, when fixed)

#### `video_generate` — Veo / Omni Flash video generator (Google)  → `video`
- **inputs (mode-dependent):**
  - image-to-video: `["Prompt", "Start Image", "End Image (optional)"]`
  - components: `["Prompt", "Ref 1", ...]`
- **outputs:** `["Video"]`.
- **values:** `mode` (`"image"` or `"components"`), `model` (server key, default `"fast"`),
  `ratio` (`VIDEO_ASPECT_RATIO_LANDSCAPE` / `VIDEO_ASPECT_RATIO_PORTRAIT`),
  `resolution` (`"720p"`, `"1080p"`, `"4K"`), `seed_mode`, `seed`.

#### `grok` — Grok image/video generator  → `any` (image or video by mode)
- **inputs (by `mode`):** `t2i`/`t2v` → `["Prompt"]`; `i2i`/`i2v` → `["Prompt", "Ref 1"]`.
- **outputs:** `["Output"]`.
- **values:**
  - `mode`: `"t2i"` | `"i2i"` | `"t2v"` | `"i2v"`
  - `ratio`: `"9:16"` | `"16:9"` | `"1:1"` | `"2:3"` | `"3:2"`
  - `video_length`: `6` | `10` | `15` (video modes only)
  - `resolution`: `"480p"` | `"720p"` (video modes only)

#### `meta` — Meta AI (vibes.ai) image/video generator  → `any`
- **inputs (by `mode`):**
  - `t2i` / `t2v` → `["Prompt"]`
...
- **values:**
  - `ratio`: `"square"` | `"landscape_3_2"` | `"landscape_4_3"` | `"landscape_16_9"` | `"portrait_2_3"` | `"portrait_3_4"` | `"portrait_9_16"`
  - `quality`: `"low"` | `"medium"` | `"high"`
  - `prompt_mode`: `"auto"` | `"direct"`
  - `reasoning`: `"none"` | `"low"` | `"medium"` | `"high"` | `"xhigh"` | `"max"`
  - `web_search`: `true` | `false`

> **`generate` / `video_generate` model note.** Their `model` and `ratio` option lists are
> **loaded from the server** and depend on the account tier, so the exact `model` data value
> isn't fixed here. The reliable way to author a Flow image/video node is to build it once in
> the UI and **Save Flow**, then reuse that node object. For fully self-contained nodes whose
> options never change, prefer **`grok`, `meta`, `openai`** — their values above are stable.
